    ...
    <div className={`${classPrefix} ${props.className}`}>
      ...
      <div className={`${classPrefix}-container`}>
        <div className={`${classPrefix}-token`}>
          ...
          <span>{token ? token.isXToken ? '取出份额' : token.symbol : <Skeleton width={80} />}</span>
        ...
        <div className={`${classPrefix}-extra`}>
          {shouldShowBalanceLoading ? (
            ...
            <>
              ...
              >
                <>
                  {`${props.balanceTitle || t`Balance:`} ${balance ? balance.amount.toFormat(8) : '--'} `}
                  {token ? (token.isXToken ? '' : token.symbol) : ''}
                </>
              </span>
